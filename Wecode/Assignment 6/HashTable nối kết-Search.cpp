...
Node * Search(Hashtable ht, int maso) {
    // Tính chỉ số bảng băm cho mã số
    int index = Hash(ht, maso);

    // Duyệt qua danh sách liên kết tại chỉ số bảng băm
    Node* p = ht.table[index].head;
    while (p != NULL) {
        // Nếu tìm thấy học sinh với mã số tương ứng, trả về con trỏ đến nút chứa học sinh
        if (p->data.Maso == maso)
            return p;

        // Chuyển sang nút tiếp theo trong danh sách
        p = p->next;
    }

    // Nếu không tìm thấy học sinh với mã số tương ứng, trả về NULL
    return NULL;
